...
## 파생특징 실험 (`src/derived_features.py`)
...
`HybridCNN` — `SmallCNN`과 동일한 conv 백본을 쓰되, flatten 출력(1024차원)에 파생특징 8개를 concat하여 분류기에 입력. `use_feats=False`로 두면 정확히 baseline과 동일한 구조가 되어, **파생특징 유무만 격리해서 비교**할 수 있음.
...
원 가설(Scratch의 선형성·저밀도를 파생특징가 보완)과 정확히 일치하는 방향으로 개선됨. Donut만 소폭 하락.
...
- 그 외 파이프라인(리사이즈, 파생특징, `HybridCNN`, 클래스 가중치)은 4단계와 동일 — **증강 유무만 격리해서 비교**.
...
`lot_num`(정수 추출) 기준으로 라벨된 172,950건을 정렬 후 10개 구간으로 나눠 불량 비율
(`failureType != 'none'`)을 계산했다.
...
초반 2개 구간(전체의 20%) 평균 0.5071 vs 나머지 8개 구간 평균 0.0576 — **약 8.8배 차이**로
...
- 모델: `HybridCNN`(CNN + 8개 파생특징), 나머지 하이퍼파라미터는 4단계와 동일.
...
(1) "증강만, 파생특징는 없이" 조합이 실험에서 누락돼 있었고, (2) CPU 스레드 비결정성으로 같은
...
validation 기준으로도 "both"(파생특징+증강)가 가장 우수해, `ablation_with_ci.py`의 결론과
일관된다. 다만 이번엔 derived(+0.0307)가 augment(+0.0026)보다 뚜렷하게 크게 나와 — 어떤 20%가
validation으로 빠지는지에 따라 "파생특징 단독 vs 증강 단독" 중 어느 쪽이 더 큰 효과인지의
...
최종 모델(`HybridCNN`)은 CNN 이미지 경로의 flatten 출력에 파생특징 8개를 concat한 뒤
분류기에 넣는 구조다. 여기서 사용하는 Grad-CAM은 CNN 경로의 마지막 conv 레이어에만 hook을
걸기 때문에, **파생특징 경로가 최종 판단에 기여한 정도는 이 기법으로 전혀 설명되지 않는다.**
...
`none`(무패턴) 사례(#40)에서도 국소적으로 강한 반응이 관찰됐으나 원인은 이번 분석 범위에서
확정하지 못했다.
